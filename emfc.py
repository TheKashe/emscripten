...
"""emfc - compiler helper script
=============================

emfc is a drop-in replacement for flang

See  emfc --help  for details.

emfc can be influenced by a few environment variables:
...
logger = logging.getLogger('emfc')

# endings = dot + a suffix, safe to test by  filename.endswith(endings)
FORTRAN_ENDINGS = ('.f','.for','.f90','.f95','.f03','.F','.F90')
OBJC_ENDINGS = ('.m', '.mi')
OBJCXX_ENDINGS = ('.mm', '.mii')
SPECIAL_ENDINGLESS_FILENAMES = ('/dev/null',)

SOURCE_ENDINGS = FORTRAN_ENDINGS + OBJC_ENDINGS + OBJCXX_ENDINGS + SPECIAL_ENDINGLESS_FILENAMES
FORTRAN_ENDINGS= FORTRAN_ENDINGS + SPECIAL_ENDINGLESS_FILENAMES # consider the special endingless filenames like /dev/null to be C
...
def save_intermediate(name, suffix='js'):
  if not DEBUG:
    return
  name = os.path.join(shared.get_emscripten_temp_dir(), 'emfc-%d-%s.%s' % (Intermediate.counter, name, suffix))
...
def log_time(name):
  """Log out times for emfc stages"""
  if DEBUG:
    now = time.time()
    logger.debug('emfc step "%s" took %.2f seconds', name, now - TimeLogger.last)
    TimeLogger.update()
...
def run(args):
  ...
  if '--help' in args:
    # Documentation for emfc and its options must be updated in:
...
emfc: supported targets: llvm bitcode, javascript, NOT elf
...
''' % (open(shared.path_from_root('site', 'build', 'text', 'docs', 'tools_reference', 'emcc.txt')).read()))
  ...
  if '--version' in args:
    ...
    try:
      ...
      os.chdir(here)
    print('''emfc (Emscripten flang replacement) %s (%s)
...
There is NO warranty; not even for MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.
  ...
  if len(args) == 1 and args[0] == '-v': # -v with no inputs
    ...
    print('emfc (Emscripten flang replacement + linker emulating GNU ld) %s' % shared.EMSCRIPTEN_VERSION, file=sys.stderr)
  ...
  def has_fortran_source(args):
    for a in args:
      if a[0] != '-' and a.endswith(FORTRAN_ENDINGS + OBJC_ENDINGS):
        return True
    return False
  ...
  if not has_fixed_language_mode:
    if not run_via_emxx and has_fortran_source(args):
      use_cxx = False
  ...
  if CONFIGURE_CONFIG or CMAKE_CONFIG:
    ...
    compiler = os.environ.get('CONFIGURE_FC')
    if not compiler:
      compiler = shared.EMFC
    ...
    if debug_configure:
      open(tempout, 'a').write('emfc, just configuring: ' + ' '.join(cmd) + '\n\n')
  ...
  FC = os.environ.get('EMMAKEN_COMPILER', shared.FLANG)
