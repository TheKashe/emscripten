  ...
  function mergeLibSymbols(libModule) {
    ...
    for (var sym in libModule) {
      ...
      if (!Module.hasOwnProperty(module_sym)) {
        err("found dylib symbol: " + module_sym);
  ...
  function loadModule() {
    ...
    var proxyHandler = {
      'get': function(obj, prop) {
        ...
        err("looking up symbol for dylib: " + prop);
        ...
        if (prop.startsWith('fp$')) {
          ...
          return obj[prop] = function() {
#if ASSERTIONS
            assert(Module[name], 'missing linked function `' + name + '`. perhaps a side module was not linked in? if this global was expected to arrive from a system library, try to build the MAIN_MODULE with EMCC_FORCE_STDLIBS=1 in the environment');
#endif
            return Module[name];
          };
        }
